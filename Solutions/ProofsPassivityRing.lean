...
theorem passive_iff_symm (N d : ℕ) [NeZero N] (hN : 3 ≤ N)
    (W : Fin N → Matrix (Fin d) (Fin d) ℝ) :
    (∀ v : Fin N → (Fin d → ℝ), power N d W v = 0) ↔ ∀ i, (W i)ᵀ = W i :=
  ⟨symm_of_passive N d hN W, fun hW v => passive_of_symm N d W hW v⟩

/-- Local-only sanity check (§5): at N = 2 the hypothesis fails and so does
the conclusion — the same non-symmetric matrix on both links gives zero power. -/
example : ∃ W : Fin 2 → Matrix (Fin 2) (Fin 2) ℝ,
    (∀ v, power 2 2 W v = 0) ∧ ¬ ∀ i, (W i)ᵀ = W i := by
  refine ⟨fun _ => !![0, 1; 0, 0], fun v => ?_, fun h => ?_⟩
  · rw [power_eq]
    simp [Fin.sum_univ_two, dotProduct, mulVec, Fin.sum_univ_two]
    ring
  · have := congrFun (congrFun (h 0) 0) 1
    simp at this
