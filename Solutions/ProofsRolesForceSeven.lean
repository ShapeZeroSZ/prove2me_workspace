...
theorem fano_has_role_colouring : ∃ role, RoleColouring fano role := by
...
theorem no_role_colouring_nine (S : STS 9) (role : Fin 9 → Finset (Fin 9) → Fin 3) :
  ...
  intro h
  have := roles_force_seven 9 (by norm_num) S role h
  omega

/-- Local-only (mission 5, section 5): the empty system satisfies every condition,
so the goal is false without `0 < n`. -/
theorem empty_system_is_counterexample :
    ∃ (S : STS 0) (role : Fin 0 → Finset (Fin 0) → Fin 3), RoleColouring S role := by
  refine ⟨⟨∅, by simp, fun x => x.elim0⟩, fun x _ => x.elim0, ?_⟩
  refine ⟨by simp, fun x => x.elim0, fun x => x.elim0⟩

theorem hypothesis_necessary :
    ¬ ∀ (n : ℕ) (S : STS n) (role : Fin n → Finset (Fin n) → Fin 3),
        RoleColouring S role → n = 7 := by
  intro H
  obtain ⟨S, role, h⟩ := empty_system_is_counterexample
  exact absurd (H 0 S role h) (by norm_num)
